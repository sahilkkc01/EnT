...
const {  Client, ClientFollowUp, ClientInteraction, ClientTeamAssignment, ClientDocument, Lead, LeadFollowUp, LeadInteraction, LeadTeamAssignment, LeadQuotation } = require('../models/MarketingSchema'); // Adjust the path based on your project structure
...
exports.saveOrUpdateClient = async (req, res) => {
    ...
    try {
        ...
        const {
            id, clientType, registrationDate, clientName, phoneNumber, secondaryPhone, email,
            address1, address2, state, city, postalCode, country, clientCategory, clientSource,
            preferredContact, preferredTime, clientRequirements, clientStatus, priority, clientSince,
            gstNumber, panNumber, internalNotes, pocDetails, serviceDetails
        } = req.body;

        // Required fields validation
        if (!clientType || !clientName || !phoneNumber || !email || !address1 || 
            !state || !city || !postalCode || !clientCategory || !clientSource || 
            !preferredContact || !clientRequirements) {
            return res.status(400).json({ success: false, message: "Missing required fields" });
        }

        // Format arrays
        const formattedPocDetails = Array.isArray(pocDetails) ? pocDetails : [];
        const formattedServiceDetails = Array.isArray(serviceDetails) ? serviceDetails : [];
...
exports.getAllClients = async (req, res) => {
    try {
        ...
        let { page = 1, limit = 50, state, city, client_name, client_status, priority, client_type } = req.query;
        ...
        if (client_name) whereCondition.clientName = { [Op.like]: `%${client_name}%` };
...
exports.updateClientStatus = async (req, res) => {
    try {
        ...
        if (!clientStatus) {
            return res.status(400).json({ success: false, message: "Client status is required" });
        }
        ...
        return res.status(200).json({ success: true, message: "Client status updated successfully" });
    } catch (error) {
...
exports.updateClientPriority = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: "Client priority updated successfully" });
    } catch (error) {
...
exports.saveClientFollowUp = async (req, res) => {
    try {console.log(req.body)
        ...
        return res.status(200).json({ success: true, message: "Follow-up saved successfully", data: followUp });
...
exports.saveClientInteraction = async (req, res) => {
    ...
    try {
        ...
        if (leadGenerated === 'Yes') {
            ...
            if (!event_type || !state || !city || !venue || !event_date || !requirement || !people_count || !lead_source || !lead_status || !priority || !next_followup) {
                await t.rollback();
...
exports.saveClientService = async (req, res) => {
    try {
        ...
        const { client_id, service_type, service_date, service_details, budget, attendees, status } = req.body;

        if (!client_id || !service_type || !service_date || !service_details || !budget || !status) {
            return res.status(400).json({ success: false, message: "Missing required fields" });
        }
        ...
        return res.status(200).json({ success: true, message: "Service saved successfully", data: service });
...
exports.assignClientTeamMember = async (req, res) => {
    try {
        ...
        const { client_id, team_member_username, team_member_name, role } = req.body;
...
exports.uploadClientDocument = async (req, res) => {
    ...
    try {
      ...
      const file_path = req.file ? path.basename(req.file.path) : existingEmployee.file_path;
...
exports.removeClientTeamMember = async (req, res) => {
    try {
        ...
        if (!clientId || !assignmentId) {
            return res.status(400).json({ success: false, message: "Client ID and Assignment ID are required" });
        }
        ...
        if (!assignment) {
            return res.status(404).json({ success: false, message: "Team assignment not found" });
        }
...
exports.getAllLeads = async (req, res) => {
    try {
        ...
        let { page = 1, limit = 50, state, city, event_type, lead_status, priority, client_id } = req.query;
        page = Math.max(1, Number(page) || 1);
        limit = Math.max(10, Number(limit) || 50); // Minimum 10 records per page
        const offset = (page - 1) * limit;
...
exports.updateLeadStatus = async (req, res) => {
    try {
        ...
        if (!lead_status) {
            return res.status(400).json({ success: false, message: "Lead status is required" });
        }
...
exports.updateLeadPriority = async (req, res) => {
    try {
        ...
        return res.status(200).json({ success: true, message: "Lead priority updated successfully" });
    } catch (error) {
...
exports.saveLeadInteraction = async (req, res) => {
    try {
        ...
        const { lead_id, interaction_type, interaction_date, notes, outcome } = req.body;
...
exports.getClientLeads = async (req, res) => {
    try {
        ...
        const leads = await Lead.findAll({
            where: { client_id: clientId, app_id: appId },
            attributes: [
                'id', 'event_type', 'venue', 'event_date',
                'lead_source', 'lead_status', 'priority',
                'next_followup', 'createdBy', 'createdAt'
            ],
            order: [['createdAt', 'DESC']],
        });
...
exports.updateClientFollowUpStatus = async (req, res) => {
    try {
        ...
        if (!updated) {
            return res.status(404).json({ success: false, message: "Follow-up not found or unauthorized" });
        }

        return res.status(200).json({ success: true, message: "Follow-up status updated successfully" });
    } catch (error) {
